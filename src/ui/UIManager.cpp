...
void UIManager::saveActiveScreenState(uint8_t displayIndex) {
    BaseScreen* screen = getActiveScreen(displayIndex);
...
void UIManager::restoreScreenState(uint8_t displayIndex, uint8_t screenId) {
    BaseScreen* screen = getScreen(displayIndex, screenId);
    if (screen) {
...
BaseScreen* UIManager::createScreen(uint8_t displayIndex, uint8_t screenId) {
    if (displayIndex == 0) {
    auto screen = std::make_unique<BaseScreen>(display1, display1Width, display1Height);
        BaseScreen* screenPtr = screen.get();
        screens1[screenId] = std::move(screen);
        return screenPtr;
    } else if (displayIndex == 1) {
    auto screen = std::make_unique<BaseScreen>(display2, display2Width, display2Height);
        BaseScreen* screenPtr = screen.get();
...
BaseScreen* UIManager::getActiveScreen(uint8_t displayIndex) {
...
BaseScreen* UIManager::getScreen(uint8_t displayIndex, uint8_t screenId) {
    if (displayIndex == 0) {
        auto it = screens1.find(screenId);
        if (it != screens1.end()) {
...
bool UIManager::update(unsigned long deltaTime) {
    ...
    BaseScreen* screen1 = getActiveScreen(0);
    if (screen1) {
    ...
    BaseScreen* screen2 = getActiveScreen(1);
    if (screen2) {
...
void UIManager::addScreen(uint8_t displayIndex, uint8_t screenId, BaseScreen* screen) {
    if (!screen) return;
    if (displayIndex == 0) {
        screens1[screenId] = std::unique_ptr<BaseScreen>(screen);
    } else if (displayIndex == 1) {
        screens2[screenId] = std::unique_ptr<BaseScreen>(screen);
    }
}
