...
void AppTitleWidget::draw(Adafruit_GFX& display) {
    ...
    // Print debug info
    Serial.println("Drawing AppTitleWidget: " + title + " / " + abbreviation);
    ...
    // Truncate title if needed to fit in the space (about 30 pixels for textSize 1)
    int maxChars = 30 / (6 * 1); // 6 pixels per character at text size 1
    if (upperTitle.length() > maxChars) {
        upperTitle = upperTitle.substring(0, maxChars);
    }
    
    // Calculate text position for centering title
    ...
    int16_t titleY = absY + 4; // Small margin from top
    ...
    // Calculate abbreviation position for centering
    ...
    int16_t abbrY = absY + titleHeight + 8; // Below title with some margin
